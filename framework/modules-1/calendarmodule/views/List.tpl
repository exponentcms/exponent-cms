...
<div class="module calendar list"> 
	...
	{br}{br}
	<a href="{link action=show_view _common=1 view='Monthly List' time=$prev_timestamp}"><img style="border:none;" src="{$smarty.const.ICON_RELATIVE|cat:'left.png'}" title="{'Prev'|gettext}" alt="{'Prev'|gettext}" /></a>
	<strong>{$time|format_date:"%B %Y"}</strong>
	<a href="{link action=show_view _common=1 view='Monthly List' time=$next_timestamp}"><img style="border:none;" src="{$smarty.const.ICON_RELATIVE|cat:'right.png'}" title="{'Next'|gettext}" alt="{'next'|gettext}" /></a>
	{br}{br}
	{foreach from=$days item=items key=ts}
		{if_elements array=$items}
			...
			{foreach from=$items item=item}
				...
				<div class="paragraph">
					<a {if $item->location_data != null}href="{link action=view id=$item->id date_id=$item->eventdate->id}"{/if} title="{$item->body|summarize:"html":"para"}">{$item->title}</a>
