...
YUI(EXPONENT.YUI3_CONFIG).use('gallery-calendar',function(Y){
	...
	new Y.Calendar('J_popup_closeable',{
        ...
        {/literal} {if ($smarty.const.SEF_URLS == 1)} {literal}
            window.location=eXp.URL_FULL+'calendarmodule/viewweek/time/'+unixtime+'/src/{/literal}{$__loc->src}{literal}';
        {/literal} {else} {literal}
            window.location=eXp.URL_FULL+'index.php?module=calendarmodule&action=viewweek&time='+unixtime+'&src={/literal}{$__loc->src}{literal}';
        {/literal} {/if} {literal}
	});
