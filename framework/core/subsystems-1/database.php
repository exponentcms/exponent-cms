...
(include_once(BASE.'framework/core/subsystems-1/database/'.DB_ENGINE.'.php')) or exit('None of the installed Exponent Database Backends will work with this server\'s version of PHP.');
...
function exponent_database_connect($username,$password,$hostname,$database,$dbclass = '',$new=false) {
	...
	(include_once(BASE.'framework/core/subsystems-1/database/'.$dbclass.'.php')) or exit('The specified database backend  ('.$dbclass.') is not supported by Exponent');
	$dbclass .= '_database';
	$newdb = new $dbclass();
