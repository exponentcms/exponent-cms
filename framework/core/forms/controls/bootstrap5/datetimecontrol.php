...
class datetimecontrol extends formcontrol {
    ...
    function toHTML($label, $name) {
        ...
        //$html .= "<label>";
        if (empty($this->flip)) {
            $html .= "<label class=\"form-label".($this->horizontal?' col-sm-2 col-form-label':'')."\">" . $label . "</label>";
    ...
    function controlToHTML($name, $label = null) {
        ...
        if ($this->showdate) {
            ...
            if ($this->showtime)
                $html .= '<label class="col-2 form-label">' . gt('Date') . ': </label>';
        ...
        if ($this->showtime) {
            ...
            if ($framework !== 'bootstrap' && $framework !== 'bootstrap3') {
                $html .= '<br /><label class="form-label form-label spacer"> </label>';
            }
            $html .= '<div class="datetime date time" style="display:inline-block;margin-left: 15px;">';
            if ($this->showdate)
                $html .= '<label class="col-2 form-label">' . gt('Time') . ': </label>';
