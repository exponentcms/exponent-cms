...
class filemanagercontrol extends formcontrol {
    ...
    function toHTML($label,$name) {
        ...
        if (USE_BOOTSTRAP_ICONS) {
            $icon_cp = 'bi bi-plus-circle';
        } else {
            $icon_cp = 'fas fa-plus-circle ';
        }
        $html .= ' <span id="adders-'.$name.'"'.$hide.'> <a class="btn btn-success '. $btn_size.'" href="#" id="addfiles-'.$name.'" title="'.gt('Add Files using the File Manager').'"><i class="'.$icon_cp.$icon_size.'"></i> '.gt('Add Files').'</a>';
        if (!$user->globalPerm('prevent_uploads')) {
        $html .= ' <a class="btn btn-success '. $btn_size.'" href="#" id="quickaddfiles-'.$name.'" title="'.gt('One-step Upload and Add Files').'"><i class="'.$icon_cp.$icon_size.'"></i> '.gt('Quick Add').'</a></span>';
        } else {
        $html .= '</span>';
        }
        $html .= '</label></div>';
                ...
                EXPONENT.batchAddFiles.".$name." = function(ids) {
                    ...
                    $.each(ids, function(k,obj){
                        if (j<limit) {
                            ...
                            $js .= "
                            html += '<a class=\" btn-danger btn-sm\" rel=\"imgdiv'+obj.id+'\" href=\"javascript:{}\" title=\"".gt('Remove this file')."\"><i class=\"" . $icon->prefix . $icon->class . " " . $icon->size . "\"></i> </a>';";
                            $js .= "
                            html += filepic;
...
//                                naturalSort.insensitive = true;
                ...
                var sortable = new Sortable(document.getElementById('filelist" . $name . "'));
            });
            ";
            ...
            expJavascript::pushToFoot(array(
                ...
                "jquery"    => 'Sortable,SimpleAjaxUploader,naturalsort',
                "content"   => $js,
             ));
        return $html;
    }
    ...
    function buildImages() {
        ...
        foreach($filearray as $val) {
            ...
            $icon = expTheme::buttonIcon('delete');
            $html .= "<a class=\"btn btn-danger btn-sm\" rel=\"imgdiv".$val->id."\" href='javascript:{}' title=\"".gt('Remove this file')."\"><i class=\"" . $icon->prefix . $icon->class . " " . $icon->size . "\"></i> </a>";
            $html .= $filepic;
