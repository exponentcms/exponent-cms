...
class datetimecontrol extends formcontrol {
    ...
    function toHTML($label, $name) {
        ...
        //$html .= "<label>";
        if (empty($this->flip)) {
            $html .= "<label class=\"control-label".($this->horizontal?' col-sm-2 col-form-label':'')."\">" . $label . "</label>";
    ...
    function controlToHTML($name, $label = null) {
        ...
        if ($this->showdate) {
            ...
            $html .= '<input class="text form-control col-1" type="text" id="' . $name . '_day" name="' . $name . '_day" aria-label="' . gt('Day') . '" size="3" maxlength="2" value="' . $default_date['mday'] . '"';
            if (!empty($this->readonly) || !empty($this->disabled)) $html .= ' disabled="disabled"';
            $html .= ' />';
            $html .= '<input class="text form-control col-1" type="text" id="' . $name . '_year" name="' . $name . '_year" aria-label="' . gt('Month') . '" size="5" maxlength="4" value="' . $default_date['year'] . '"';
            if (!empty($this->readonly) || !empty($this->disabled)) $html .= ' disabled="disabled"';
            $html .= ' />';
            $html .= '</div>';
        }
        if ($this->showtime) {
            ...
            $html .= '<input class="text timebox form-control col-1" type="text" id="' . $name . '_hour" name="' . $name . '_hour" aria-label="' . gt('Hour') . '" size="3" maxlength="2" value="' . $hour . '"';
            if (!empty($this->readonly) || !empty($this->disabled)) $html .= ' disabled="disabled"';
            $html .= ' />';
            $html .= '<input class="text timebox form-control col-1" type="text" id="' . $name . '_minute" name="' . $name . '_minute" aria-label="' . gt('Minute') . '" size="3" maxlength="2" value="' . $minute . '"';
            if (!empty($this->readonly) || !empty($this->disabled)) $html .= ' disabled="disabled"';
            $html .= ' />';
            $html .= '<select class="select form-control col-1" id="' . $name . '_ampm" name="' . $name . '_ampm" size="1"';
            if (!empty($this->readonly) || !empty($this->disabled)) $html .= ' disabled="disabled"';
            $html .= '>';
