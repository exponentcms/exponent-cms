...
function build_menu($page, $params) {
    ...
    } elseif (bs5() && USE_BOOTSTRAP_ICONS && !empty($page->glyph)) {
    ...
    if ((empty($page->itemdata) && empty($page->submenu) && (empty($page->type) || (!empty($page->type) && $page->type != 3))) || $page->depth + 1 == $params['length']) {  // this is a menu item
        ...
        if (!empty($page->depth))
            $menu .= '"><a href="'.$page->url.'" data-bs-target="dropdownMenu' . $page->id . '" class="dropdown-item dropdown-toggle' . $active . '" href="'.$page->url.'" data-bs-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"'.($page->new_window?' target="_blank"':'').' title="'. $page->title .'">' . $menu_item;
        else {
