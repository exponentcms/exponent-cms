...
if (!function_exists('smarty_function_getchromemenu')) {
    ...
    //		$grouplink = $router->makeLink(array('module'=>expModules::getControllerName($module->info['class']), 'src'=>$module->info['source'], 'action'=>'groupperms', '_common'=>1));
            ...
            $list .= '<li role="menuitem"><a href="'.$userlink.'" class="dropdown-item"><i class="'.(USE_BOOTSTRAP_ICONS?'bi-person':'fas fa-user fa-fw').'"></i> '.gt("User Permissions").'</a></li>';
            $list .= '<li role="menuitem"><a href="'.$grouplink.'" class="dropdown-item"><i class="'.(USE_BOOTSTRAP_ICONS?'bi-people':'fas fa-users fa-fw').'"></i> '.gt("Group Permissions").'</a></li>';
        }
    ...
    //            $editlink = $router->makeLink(array('module'=>'containermodule', 'id'=>$module->id, 'action'=>'edit', 'src'=>$module->info['source']));
                ...
                $list .= '<li role="menuitem"><a href="'.$editlink.'" class="dropdown-item"><i class="'.(USE_BOOTSTRAP_ICONS?'bi-gear':'fas fa-cogs fa-fw').'"></i> '.gt("Configure Action")." &amp; ".gt("View").'</a></li>';
            }
        }
        ...
        if ($module->permissions['configure'] == 1) {
            if (expModules::controllerExists($module->info['class'])) {
                ...
                $list .= '<li role="menuitem"><a href="'.$configlink.'" class="config-mod dropdown-item"><i class="'.(USE_BOOTSTRAP_ICONS?'bi-gear':'fas fa-cogs fa-fw').'"></i> '.gt("Configure Settings").'</a></li>';
    ...
    //		$deletelink = $router->makeLink(array('module'=>'containermodule', 'id'=>$module->id, 'action'=>'delete', 'rerank'=>$rerank));
            ...
            $list .= '<li role="menuitem"><a href="'.$deletelink.'" class="dropdown-item" onclick="return confirm(\''.gt("Remove this module and send the content to the Recycle Bin to be recovered later?").'\')"><i class="'.(USE_BOOTSTRAP_ICONS?'bi-x-circle':'fas fa-times-circle fa-fw').'  text-danger"></i> '.gt("Remove Module").'</a></li>';
        }

        // does it need a help menu item?
        if (HELP_ACTIVE) {
            $helplink = help::makeHelpLink(expModules::getControllerName($module->info['class']));
            $list .= '<li role="menuitem"><a href="'.$helplink.'" class="dropdown-item" target="_blank"><i class="'.(USE_BOOTSTRAP_ICONS?'bi-question':'fas fa-question fa-fw').'"></i> '.gt("Get Help").'</a></li>';
        }
