 ...
 * @param \Smarty $smarty
 */
if (!function_exists('smarty_function_control')) {
                ...
                case "dropdown":
                    ...
                    if (isset($params['from']) && isset($params['to'])) {
                        ...
                        if (is_array($params['items'])) {
                            ...
                            if (!empty($params['values'])) {
                                $values = is_array($params['values']) ? $params['values'] : explode(
                                    $delimiter,
                                    $params['values']
                                );
                            } else {
...
//                break;
                ...
                case "state":
                    ...
                    if ($db->tableExists('geo_region')) {
                        ...
                        $states = $db->selectObjects(
                            'geo_region',
                            'country_id=' . $country . ' AND active=1 ORDER BY rank, name ASC'
                        );
                        ...
                        if (!empty($params['value']) && !is_numeric($params['value']) && !is_array($params['value'])) {
                            $params['value'] = $db->selectValue(
                                'geo_region',
                                'id',
                                'name="' . $params['value'] . '" OR code="' . $params['value'] . '"'
                            );
                        }
                    } else {
                        echo "NO TABLE";
                ...
                case "antispam":
                    ...
                    if (SITE_USE_ANTI_SPAM && ANTI_SPAM_CONTROL == 'recaptcha') {
                        ...
                        if (!defined('RECAPTCHA_PUB_KEY') || RECAPTCHA_PUB_KEY == '') {
                            echo '<h2 style="color:red">' . gt(
                                    'reCaptcha configuration is missing the public key.'
                                ) . '</h2>';
                            return;
                        }
...
//        $badvals = array("[", "]", ",", " ", "'", "\"", "&", "#", "%", "@", "!", "$", "(", ")", "{", "}");
            ...
            */
            ...
            echo '<h2 style="color:red">' . gt(
                    "Both the 'type' and 'name' parameters are required for the control plugin to function"
                ) . '</h2>';
        }
    }
}
