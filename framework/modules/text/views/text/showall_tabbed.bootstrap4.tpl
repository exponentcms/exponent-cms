    ...
    <div id="text-{$id}" class="">
        <ul class="nav nav-tabs" role="tablist">
            {foreach from=$items item=tab name=tabs}
                <li role="presentation" class="nav-item"><a href="#tab{$smarty.foreach.tabs.iteration}-{$id}" class="nav-link{if $smarty.foreach.tabs.iteration == 1} active{/if}" role="tab" data-toggle="tab">{if $tab->title ==""}&#160;{else}{$tab->title}{/if}</a></li>
            {/foreach}
            {permissions}
                {if ($permissions.create)}
                    <li role="presentation" class="nav-item"><a href="#tab{$smarty.foreach.tabs.iteration+1}-{$id}" class="nav-link{if $smarty.foreach.tabs.iteration == 0} active{/if}" role="tab" data-toggle="tab"><em>({'Add New'|gettext})</em></a></li>
                {/if}
            {/permissions}
        </ul>
