    ...
    <div id="text-{$id}" class="">
        <ul class="nav nav-tabs" role="tablist">
            {foreach from=$items item=tab name=tabs}
                <li role="presentation"{if $smarty.foreach.tabs.iteration == 1} class="active"{/if}><a href="#tab{$smarty.foreach.tabs.iteration}" role="tab" data-toggle="tab">{if $tab->title ==""}&#160;{else}{$tab->title}{/if}</a></li>
            {/foreach}
            {permissions}
                {if ($permissions.create)}
