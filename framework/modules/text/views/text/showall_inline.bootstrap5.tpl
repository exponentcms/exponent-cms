            ...
            $.ajax({
                ...
                success:function(msg) {
                    if (msg.replyCode == '200') {
                        ...
                        newItem += '<a class="btn btn-default {/literal}{$btn_size}{literal}" title="{/literal}{'Edit this text item'|gettext}{literal}" href="' + EXPONENT.PATH_RELATIVE + 'text/edit/id/' + data.id + '/src/' + src + '"><i class="{/literal}{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-pencil-square{else}fas fa-edit{/if} {$icon_size}{literal}"></i> {/literal}{'Edit'|gettext}{literal}</a>';
