...
{else}
    {$maxwidth = $config.maxwidth}
{/if}
{$dropsize = "dropdown_`$maxwidth`column"|plural:$maxwidth}
                            ...
                            {function menu_items depth = 0}
                                ...
                                {if empty($child->submenu)}
                                    <div class="col_{$width}">
                                        <a {$class}{if $child->url != "#"}href="{$child->url}"{/if}{if $child->new_window} target="_blank"{/if}><h3>{if !empty($child->expFile[0]->id)}{img class=img_left file_id=$child->expFile[0]->id w=24 h=24}{/if}{if $config.usetitle && !empty($child->title)}{$child->title}{else}{$child->text}{/if}</h3>{if $config.usedesc}{$description}{/if}</a>
                                    </div>
                                {else}
                                    <div class="col_{$width}">
                                        <a {$class}{if $child->url != "#"}href="{$child->url}"{/if}{if $child->new_window} target="_blank"{/if}><h3>{if !empty($child->expFile[0]->id)}{img class=img_left file_id=$child->expFile[0]->id} {/if}{if $config.usetitle && !empty($child->title)}{$child->title}{else}{$child->text}{/if}</h3>{if $config.usedesc}{$description}{/if}</a>
                                    </div>
                                    {*{if $depth > 0}*}
                                        {*<div class="col_{$depth}">&#160;</div>*}
                                    {*{/if}*}
                                    {foreach from=$child->submenu->itemdata item=submenu}
                                        ...
                                        {else}
                                            <div class="col_1">
                                                <a {$class}{if $submenu->url != "#"}href="{$submenu->url}"{/if}{if $submenu->new_window} target="_blank"{/if}><h4>{if !empty($submenu->expFile[0]->id)}{img class=img_left file_id=$submenu->expFile[0]->id} {/if}{if $config.usetitle && !empty($submenu->title)}{$submenu->title}{else}{$submenu->text}{/if}</h4>{if $config.usedesc}{$description}{/if}</a>
                                            </div>
                                        {/if}
                                    {/foreach}
                                {/if}
