...
{permissions}
                ...
                <div class="dropdown exp-container-chrome exp-container-chrome-container">
                    ...
                    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu{$top->id}">
                        ...
                        {if $user->isAdmin() && !$smarty.const.SIMPLE_PERMISSIONS}
                            ...
                            <li role="menuitem"><a href="{link controller=users action=groupperms mod=container}" class="dropdown-item"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-people{else}fas fa-users fa-fw{/if}"></i> {"Group Permissions"|gettext}</a></li>
                        {/if}
                        ...
                        {if ($permissions.configure || $container->permissions.configure)}
                            <li role="menuitem"><a href="{link module=container src=$module->info['source'] action='configure' hcview=$top->view}" class="config-mod dropdown-item"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-gear{else}fas fa-cogs fa-fw{/if}"></i> {"Configure Settings"|gettext}</a></li>
                        {/if}
                        {if $smarty.const.HELP_ACTIVE}<li role="menuitem"><a href="{help::makeHelpLink('container')}" class="dropdown-item" target="_blank"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-question{else}fas fa-question fa-fw{/if}"></i> {'Get Help'|gettext}</a></li>{/if}
                    </ul>
                </div>
            </div>
        {/if}
	...
	{if $permissions.create && empty($hidebox)}
		<a class="exp-addmodule-link" href="{link action=edit rerank=1 rank=1}"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-plus{else}fas fa-plus fa-fw{/if}"></i> {"Add Module"|gettext}</a>
	{/if}
{/permissions}
	...
	{else}
		...
		{permissions}
			{if $permissions.create && $hidebox == 0}
				<a class="exp-addmodule-link" href="{link action=edit rerank=1 rank=$smarty.foreach.c.iteration+1}"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-plus{else}fas fa-plus fa-fw{/if}"></i> {"Add Module"|gettext}</a>
			{/if}
		{/permissions}
	{/if}
{/foreach}
