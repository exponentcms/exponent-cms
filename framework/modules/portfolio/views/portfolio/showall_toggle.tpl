...
YUI(EXPONENT.YUI3_CONFIG).use('*', function(Y) {
    ...
    {/literal}
        {$orig_params = ['controller' => 'portfolio', 'action' => 'showall', 'view' => $params.view, 'src' => $params.src]}
    {literal}
	...
	var sUrl = EXPONENT.PATH_RELATIVE+"index.php?controller=portfolio&action=showall&view=portfoliolist_toggle&ajax_action=1&src={/literal}{$__loc->src}{literal}";
    ...
    portfoliolist.delegate('click', function(e){
        e.halt();
        History.pushState({name:'{/literal}{$name}{literal}',rel:e.currentTarget.get('rel')}, '{/literal}{'Portfolio'|gettext}{literal}', orig_url+page_parm+e.currentTarget.get('rel'));
