...
<div class="module imagegallery showall-slideshow">
						...
						{permissions}
							<div class="item-actions">
								...
								{if $permissions.delete == 1}
									{icon action=delete record=$record title="Delete `$record->title`" onclick="return confirm('Are you sure you want to delete this `$modelname`?');"}
								{/if}
								{if $permissions.create == 1}
									{icon class="add addhere" action=edit rank=`$text->rank+1` title="Add another slide here"  text="Add another slide here"|gettext}
								{/if}
							</div>
						{/permissions}
					</div>
    ...
    {if $trucount>1 && $config.pa_show_controls}
		<div class="slideshow-buttons">
			<a id="prev{uniqueid}" href="#" class="prev_slide" title="Prevous Slide"|gettext>
				&lt;&lt; Previous</a>
			<a id="plps{uniqueid}" href="#" class="pause_slide" title="Play/Pause Slideshow"|gettext>
				Pause</a>
			<a id="next{uniqueid}" href="#" class="next_slide" title="Next Slide"|gettext>
				Next &gt;&gt;
			</a>
		</div>
    {/if}
