...
<p>Log in to the Twitter Developer's <a href="https://dev.twitter.com/apps" target="_blank">website</a> with your twitter account.{br}
First create an application which will provide you the Consumer key and secret.{br}
Then you must create an Access token which will give you the Access token settings.{br}
<b>Give your application 'read' & 'write' access before requesting a token to also create tweets</b></p>
...
{control type=checkbox name="showattrib" value=1 label="Show attribution?" checked=$config.showattrib|default:0}
{control type=checkbox name="showimage" value=1 label="Show user image?" checked=$config.showimage|default:0}
