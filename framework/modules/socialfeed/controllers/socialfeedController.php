...
//        if (empty($message_feed['full_feed_link']))  //fixme we won't get here?
    ...
    {
        $facebook_latest = $this->get_facebook_feed($this->config['socialfeed_facebook_page_name']);
                ...
                if (isset($twitter_values) && !empty($twitter_values)) {
                    if (array_key_exists('errors', $twitter_values)) {
                        if ($twitter_values["errors"][0]["message"] != "") {
                            eLog("<h3>Sorry, there was a problem.</h3><p>Twitter returned the following error message:</p><p><em>" . $twitter_values[errors][0]["message"] . "</em></p>");
                            exit();
                        }
                    }
...
//        }
    ...
    private function socialfeed_pinterest_posts()
    {
        ...
        $rss4->load('http://pinterest.com/'.$pinterest_pull);
