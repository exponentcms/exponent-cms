    ...
    function showall() {
        ...
        if (!empty($this->params['past']) && $user->isAdmin()) {
            ...
            if ($user->isAdmin()) {
                ...
                $events = $this->eventregistration->find('all', 'product_type=\'eventregistration\' AND (active_type=0 OR active_type IS NULL)', "title ASC", $limit);
            }
            foreach ($events as $event) {
//                if ($user->isAdmin()) {
...
//            $dates = $er->find('all', "(eventdate >= " . expDateTime::startOfDayTimestamp($start) . " AND eventdate <= " . expDateTime::endOfDayTimestamp($start) . ")");
            ...
            if ($user->isAdmin()) {
                ...
                $events = $er->find('all', 'product_type=\'eventregistration\' && (active_type=0 OR active_type IS NULL)', "title ASC");
            }
            $dates = array();
...
//            }
            if (!empty($f->is_saved)) {
                ...
                foreach ($registrants as $key=>$registrant) {
                    $registrants[$key]->registered_date = date(strftime_to_date_format(DISPLAY_DATETIME_FORMAT), $registrants[$key]->registered_date);
                }
            }
...
//                'text_message'=> strip_tags(str_replace("<br>", "\r\n", $this->params['email_message'])),
    ...
    static function getRegEventsForDates($startdate, $enddate, $color="#FFFFFF") {
        $er = new eventregistration();
        $events      = $er->find('all', 'product_type=\'eventregistration\' && (active_type=0 OR active_type IS NULL)');
        $pass_events = array();
        foreach ($events as $event) {
