...
class groupdiscounts extends expRecord {
    ...
    public static function getGroupDiscountsForUser() {
        ...
        $sql .= 'FROM ' . $db->tableStmt('group') . ' g JOIN ' . $db->tableStmt('groupmembership') . ' gm ';
        $sql .= 'ON g.id=gm.group_id JOIN ' . $db->tableStmt('groupdiscounts') . ' gd ON gd.group_id=gm.group_id ';
        $sql .= 'JOIN ' . $db->tableStmt('discounts') . ' d ON d.id=gd.discounts_id WHERE gm.member_id='.$user->id;
