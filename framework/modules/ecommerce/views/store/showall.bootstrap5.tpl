...
{css unique="storeListing" link="`$asset_path`css/storefront_bs4.css"}
...
<div class="module store showall">
    ...
    {if $current_category->id}
        {permissions}
            <div class="module-actions">
                {if $permissions.edit}
                    {icon action=edit module=storeCategory record=$current_category title="Edit `$current_category->title`" text="Edit this Store Category"|gettext}
                {/if}
                {if $permissions.manage}
    ...
    {if !$categories|@count || $config.show_products}
        ...
        <div class="row">
            ...
            <div class="btn-group pull-right list-grid">
                <a href="#" id="list" class="btn btn-default btn-sm"><span class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-list-ul{else}fas fa-th-list{/if}"></span> {'List'|gettext}</a>
                <a href="#" id="grid" class="btn btn-default btn-sm"><span class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-grid-3x3{else}fas fa-th{/if}"></span> {'Grid'|gettext}</a>
            </div>
        </div>
        ...
        <div id="products" class="products row view-group">
