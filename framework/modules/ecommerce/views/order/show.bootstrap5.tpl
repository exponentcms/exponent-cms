...
{css unique="showorder" link="`$asset_path`css/ecom-bs4.css" corecss="tables"}
{literal}
    ...
    <div class="col-lg-12">
        <div class="card order-stats">
            <div class="row">
                ...
                <div class="col-6 col-sm-3 box-stats color3">
                    <div class="kpi-content">
                        <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-calendar3{else}far fa-calendar-alt{/if}"></i>
                        <span class="title">{'Date'|gettext}</span>
                        <span class="value">{$order->purchased|format_date:"%b %e, %Y"}</span>
                    </div>
                </div>
                <div class="col-6 col-sm-3 box-stats color4">
                    <div class="kpi-content">
                        <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-cash{else}far fa-money-bill-alt{/if}"></i>
                        <span class="title">{'Total'|gettext}</span>
                        <span class="value">{$order->grand_total|currency}</span>
                    </div>
                </div>
                <div class="col-6 col-sm-3 box-stats color2">
                    <a href="#start_messages">
                        <div class="kpi-content">
                            <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-chat{else}fas fa-comments{/if}"></i>
                            <span class="title">{'Messages'|gettext}</span>
                            <span class="value">{expSimpleNote::noteCount($order->id, "order")}</span>
                        </div>
                    </a>
                </div>
                <div class="col-6 col-sm-3 box-stats color1">
                    <a href="#start_products">
                        <div class="kpi-content">
                            <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-book{else}fas fa-book{/if}"></i>
        ...
        <div class="row">
            ...
            <div class="col-lg-7">
                <div class="card">
                    <div class="card-header">
                        <!-- invoice # browser -->
                        <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-credit-card{else}fas fa-credit-card{/if}"></i>
                        ...
                        <div class="card-header-action">
                            <div class="btn-group">
                                <a class="btn btn-default" href="{link action=show invoice=$order->invoice_id -1}" title="{'Previous Invoice'|gettext}">
                                    <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-skip-backward-fill{else}fas fa-backward{/if}"></i>
                                </a>
                                <a class="btn btn-default" href="{link action=showall}" title="{'All Invoices'|gettext}">
                                    <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-eject-fill{else}fas fa-eject{/if}"></i>
                                </a>
                                <a class="btn btn-default" href="{link action=show invoice=$order->invoice_id +1}" title="{'Next Invoice'|gettext}">
                                    <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-skip-forward-fill{else}fas fa-forward{/if}"></i>
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <ul id="tabOrder" class="nav nav-tabs" role="tablist">
                        <li role="presentation" class="nav-item">
                            <a href="#status" class="nav-link active" role="tab" data-bs-toggle="tab">
                                <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-clock{else}far fa-clock{/if}"></i>
                                {'Status'|gettext}
                                <span class="badge">
                        ...
                        <li role="presentation" class="nav-item">
                            <a href="#other" class="nav-link" role="tab" data-bs-toggle="tab">
                                <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-people{else}fas fa-users{/if}"></i>
                                {'Sales Reps and Referrers'|gettext}
                            </a>
                        </li>
                    </ul>
                    <div class="tab-content card">
                        <div id="status" role="tabpanel" class="tab-pane fade show active">
                                    ...
                                    <table class="table order-info order-info2">
                                        <thead id="change-status">
                                            <tr>
                                                ...
                                                <th title="{'Click to change order type or status'|gettext}"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-chevron-double-down{else}fas fa-angle-double-down{/if}"></i> {'Update Order Type and Order Status'|gettext}</th>
                                            </tr>
                                        </thead>
                    ...
                    <ul id="myTab" class="nav nav-tabs" role="tablist">
                        <li role="presentation" class="nav-item">
                            <a href="#shipping" class="nav-link active" role="tab" data-bs-toggle="tab">
                                <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-truck{else}fas fa-truck{/if}"></i>
                                {'Shipping'|gettext}
                            </a>
                        </li>
                        <li role="presentation" class="nav-item">
                            <a href="#tracking" class="nav-link" role="tab" data-bs-toggle="tab">
                                <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-upc{else}fas fa-ticket-alt{/if}"></i>
                                {'Tracking'|gettext}
                            </a>
                        </li>
                    </ul>
                    <div class="tab-content card">
                        <div id="shipping" role="tabpanel" class="tab-pane fade show active">
                            {permissions}
            ...
            <div class="col-lg-5">
                ...
                <div class="card">
                    <div class="card-header">
                        <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-person{else}fas fa-user{/if}"></i>
                        {'Customer'|gettext}
                        <span class="badge">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <dl class="card card-body bg-faded list-detail">
                                <dt>
                                    {'Email'|gettext}
                                </dt>
                                <dd>
                                    <a href="mailto:{$order_user->email}" title="{'Email customer'|gettext}"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-envelope{else}far fa-envelope{/if}"></i> {$order_user->email}</a>
                                </dd>
                                <dt>
                                    {'Account Registered'|gettext}
                                </dt>
                                <dd class="text-muted">
                                    <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-calendar3{else}far fa-calendar-alt{/if}"></i> {$order_user->created_on|format_date:$smarty.const.DISPLAY_DATETIME_FORMAT}
                                </dd>
                                <dt>
                                    {'Last Visit'|gettext}
                                </dt>
                                <dd class="text-muted">
                                    <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-calendar3{else}far fa-calendar-alt{/if}"></i> {$order_user->last_login|format_date:$smarty.const.DISPLAY_DATETIME_FORMAT}
                                </dd>
                                <dt>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <ul id="tabAddresses" class="nav nav-tabs" role="tablist">
                                <li role="presentation" class="nav-item">
                                    <a href="#addressShipping" class="nav-link active" role="tab" data-bs-toggle="tab">
                                        <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-truck{else}fas fa-truck{/if}"></i>
                                        {'Shipping Address'|gettext}
                                    </a>
                                </li>
                                <li role="presentation" class="nav-item">
                                    <a href="#addressInvoice" class="nav-link" role="tab" data-bs-toggle="tab">
                                        <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-file-text{else}fas fa-file-alt{/if}"></i>
                                        {'Invoice Address'|gettext}
                                    </a>
                                </li>
                            </ul>
                            <div class="tab-content card">
                                <div id="addressShipping" role="tabpanel" class="tab-pane fade show active">
                                    <div class="card card-body bg-faded">
                                        <div class="row">
                                            <div class="col-sm-6">
                                                <!-- shipping address -->
                ...
                <div class="card">
                <!-- messages/notes -->
                    <div id="start_messages" class="card-header">
                        <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-envelope{else}fas fa-envelope{/if}"></i> {'Messages'|gettext}
                        <span class="badge">
                    ...
                    <div class="card card-body bg-faded">
                        <div class="email-customer"><h4 id="email-customer" title="{'Click to create customer email'|gettext}"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-chevron-double-down{else}fas fa-angle-double-down{/if}"></i> {"Email the Customer about this order"|gettext}</h4></div>
        ...
        <div id="start_products" class="row">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-cart{else}fas fa-shopping-cart{/if}"></i>
                        {'Products'|gettext}
                        <span class="badge">
