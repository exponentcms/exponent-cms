			...
			<tbody>
				{foreach from=$page->records item=listing name=listings}
                    <tr class="{cycle values="odd,even"}">
                        ...
                        <td>{if $listing->order_status_id == $new_order}<span style="padding:3px;border-radius:5px;background-color:darkseagreen;">{/if}{$listing->status}{if $listing->order_status_id == $new_order}</span>{/if}</td>
