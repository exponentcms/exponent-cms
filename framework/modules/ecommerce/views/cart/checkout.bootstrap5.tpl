    ...
    {if ecomconfig::getConfig('policy')!=""}
        <div>
            ...
            {*<div id="storepolicies" style="z-index:9999">*}
                ...
                {*</div>*}
            {*</div>*}
            {*script unique="policypop" jquery="bootstrap-dialog" bootstrap="modal,tab"}
            {literal}
                    ...
                    {if $shipping->splitshipping == false}
                        ...
                        <div class="shipping-address col-4">
                            ...
                            <div class="bracket{if !$shipping->address->id} hide{/if}">
                                ...
                                {capture assign="link"}{link controller=address action=myaddressbook ajax_action=1}{/capture}
                    ...
                    <div id="cart-{$id}" class="">
                        <ul class="nav nav-tabs" role="tablist">
                            {foreach from=$billing->calculator_views item=cviews name=tabs}
                                <li role="presentation" class="nav-item"><a href="#tab{$smarty.foreach.tabs.iteration}" class="nav-link{if $smarty.foreach.tabs.first} active{/if}" role="tab" data-bs-toggle="tab" title="{'Select this payment method'|gettext}">{$billing->selectable_calculators[$cviews.id]}</a></li>
                            {/foreach}
                        </ul>
