	    ...
	    <div id="edituser-tabs" class="">
		    <ul class="nav nav-tabs" role="tablist">
                ...
                {if !empty($groups->records)}
                    <li role="presentation" class="nav-item"><a href="#tab2" class="nav-link" role="tab" data-bs-toggle="tab"><em>{"Group Membership"|gettext}</em></a></li>
                {/if}
                {foreach from=$extensions item=extension}
		            <li role="presentation" class="nav-item"><a href="#tab{$extension->id+2}" class="nav-link" role="tab" data-bs-toggle="tab"><em>{$extension->title}</em></a></li>
                {/foreach}
		    </ul>
	            ...
	            <div id="tab1" role="tabpanel" class="tab-pane fade show active">
	                ...
	                {if $user->isAdmin()}
                        {if $smarty.const.USE_LDAP}
                            {control type=checkbox name=is_ldap value=1 label="Use LDAP Authentication?"|gettext checked=$edit_user->is_ldap postfalse=1}
                        {/if}
