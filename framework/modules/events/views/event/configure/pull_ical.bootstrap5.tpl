...
<div id="icalpullControl" class="control">
    ...
    {else}
        ...
        {/if}
    {/if}
    <a id="addtoicallist" class="btn btn-success {$btn_size}" href="#"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-plus-circle{else}fas fa-plus-circle{/if} {$icon_size}"></i> {'Add to list'|gettext}</a>{br}{br}
        ...
        {foreach from=$config.pull_ical item=feed name=feed}
            {if $feed!=""}
                <li>
                    ...
                    <a class="removeical btn {$btn_size} btn-danger" href="#"><i class="{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-x-circle{else}fas fa-times-circle{/if} {$icon_size}"></i> {"Remove"|gettext}</a>
                </li>
            {/if}
        {/foreach}
    ...
    YUI(EXPONENT.YUI3_CONFIG).use('*', function(Y) {
        ...
        Y.one('#addtoicallist').on('click', function(e){
            ...
            newRemove.innerHTML = " {/literal}<i class='{/literal}{if $smarty.const.USE_BOOTSTRAP_ICONS}bi-x-circle{else}fas fa-times-circle{/if}{literal} {$icon_size}'></i> {'Remove'|gettext}{literal}";
