...
<div class="module flowplayer mediaplayer showall-playlist">
    {if $moduletitle && !($config.hidemoduletitle xor $smarty.const.INVERT_HIDE_TITLE)}<h1>{$moduletitle}</h1>{/if}
    {permissions}
   		<div class="module-actions">
   			{if $permissions.manage == 1}
   				{ddrerank items=$page->records model="media" label="Media piece"|gettext}
   			{/if}
   		</div>
   	{/permissions}
    {if $config.moduledescription != ""}
   	...
   	{/if}
    <a id="playlist-player" href="{$page->records[0]->expFile.media[0]->url}" style="display:block;width:{$config.video_width}px;height:{$config.video_height}px;">
    ...
    <ul>
        ...
        {/permissions}
		{foreach name=items from=$page->records item=media}
			<div class="item">
				<li><a class="li-link" href="#" title="{'Play this Media piece'|gettext}" onclick="swapvideo('{$media->expFile.media[0]->url}')">{$media->title}</a>
					{permissions}
						<div class="item-actions">
							{if $permissions.edit == 1}
								{icon action=edit record=$media title="Edit `$media->title` media piece"}
							{/if}
							{if $permissions.delete == 1}
								{icon action=delete record=$media title="delete `$media->title` media piece"}
							{/if}
						</div>
					{/permissions}
                    {if $media->body}
