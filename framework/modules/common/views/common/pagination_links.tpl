...
{if $page->total_records > 0}
    <div class="pagination-links" aria-label="{'Page navigation'|gettext}">
        <span class="pagination">
            {if $page->previous_page != ''}<a class="pager prev" href="{$page->previous_page}" rel="{$page->previous_pagenum}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a> {/if}
            ...
            {if $page->next_page != ''} <a class="pager next" href="{$page->next_page}" rel="{$page->next_pagenum}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>{/if}
        </span>
