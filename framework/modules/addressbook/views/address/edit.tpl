...
<div class="module address edit address-form">
    ...
    {form action=update}
        ...
        {if $user->is_admin || $user->is_acting_admin}
            {control type=state name=state label="<span class=\"required\">*</span>State" includeblank="-- Choose a State -- " all_us_territories=true exclude="6,8,10,17,30,46,50" value=$record->state add_other=true}
        ...
        {else}
            {control type=state name=state label="<span class=\"required\">*</span>State" includeblank="-- Choose a State -- " value=$record->state all_us_territories=true exclude="6,8,10,17,30,46,50"}
            {control type=hidden name=country value=223}
        {/if}
        
        {control type=text name=zip label="<span class=\"required\">*</span>Zip Code" value=$record->zip}
