...
{css unique="newui" scssprimer="`$smarty.const.PATH_RELATIVE`external/bootstrap4/scss/newui.scss" lessvars=$lessvars}
...
{function name=menu level=0}
    {if is_array($data.submenu)}
        <li class="dropdown-submenu">
            <a id="dropdownMenu{$data.submenu.id}" class="dropdown-item" data-toggle="dropdown" href="#">{if $data.icon5}<i class="{$data.icon5} fa-fw"></i>{/if} {$data.text}</a>
    ...
    {else}
        ...
        {else}
            <li role="menuitem">
                <a id="{$data.id}" href="{$data.url|default:'#'}" class="dropdown-item">{if $data.icon5}<i class="{$data.icon5} fa-fw"></i>{/if} {$data.text}</a>
            </li>
        {/if}
        {if $data.divider}
...
<div class="exp-skin">
    <header id="admin-toolbar" class="navbar navbar-expand-sm navbar-dark bg-dark fixed-{if $top}top{else}bottom{/if}" role="navigation">
        <!-- Brand and toggle get grouped for better mobile display -->
        {*<div class="navbar-header">*}
            {*<button type="button" class="navbar-toggle navbar-toggler hidden-sm-up" data-toggle="collapse"*}
        ...
        <div class="collapse navbar-collapse" id="admin-navbar-collapse-1">
            {foreach from=$menu item=topnav name=tbmenu}
                <ul class="nav navbar-nav{if $topnav.alignright} ml-auto{/if}">
                    <li class="dropdown nav-item">
                        <a href="#" id="dropdownMenu{$topnav.icon}" class="dropdown-toggle nav-link" data-toggle="dropdown">{if $topnav.icon5}<i class="{$topnav.icon5} fa-fw"></i>{/if} {$topnav.text} </a>
