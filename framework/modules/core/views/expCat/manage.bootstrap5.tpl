...
<div class="module expcat manage">
    ...
    <div id="{$id}" class="">
        <ul class="nav nav-tabs" role="tablist">
            {if !empty($page)}
                <li role="presentation" class="nav-item"><a href="#tab0" class="nav-link active" role="tab" data-bs-toggle="tab">{$page->model|capitalize} {'Items'|gettext}</a></li>
            {/if}
            {foreach name=tabs from=$cats->modules key=moduleid item=module}
                <li role="presentation"  class="nav-item"><a href="#tab{$smarty.foreach.tabs.iteration}" class="nav-link{if $smarty.foreach.tabs.first && empty($page)} active{/if}" role="tab" data-bs-toggle="tab">{$moduleid|capitalize} {$catnames}</a></li>
            {foreachelse}
                <li role="presentation" class="nav-item"><a href="#tab0" class="nav-link active" role="tab" data-bs-toggle="tab">{if $model == 'file'}{'No Folders Defined'|gettext}{else}{'No Categories Defined'|gettext}{/if}</a></li>
            {/foreach}
        </ul>
