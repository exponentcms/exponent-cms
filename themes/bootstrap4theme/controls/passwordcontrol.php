...
//        if ($this->required) $label = "*" . $label;
        ...
        if ($this->type != 'hidden') {
            ...
            if(empty($this->flip)){
                    $html .= empty($label) ? "" : "<label".$for." ".((bs3()||bs4())?"class=\"control-label\"":"").(($this->horizontal)?"col-sm-2 control-label":"" ).">". $labeltag."</label>";
            ...
            toggleMask: false,
//            mainTemplate: '<div class=\"kv-strength-container\">{input}<span class=\"kv-meter-container float-left\">{meter}</span></div>',
            rules: {
