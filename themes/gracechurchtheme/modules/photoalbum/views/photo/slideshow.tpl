            ...
            {foreach key=key from=$slides item=slide name=slides}
                <li class="slide"{if !$config.hidetext} data-plugin-slide-caption='<{$config.item_level|default:'h2'}>{$slide->title|escape}</{$config.item_level|default:'h2'}> {$slide->body|escape}'{/if}>
                    ...
                    {*{else}*}
                        ...
                        <!-- <img src="/external/phpThumb/phpThumb.php?src={$slide->expFile[0]->path_relative}&w=1500&h=500&far=1" class="slide-image" /> -->
                        <img src="/external/phpThumb/phpThumb.php?src={$slide->expFile[0]->path_relative}&w=1349&h=449&far=TL&f=jpeg&zc=1" class="slide-image" />
...
{literal}
    $(function () {
        $("#ss-{/literal}{$name}{literal} .slideshow-frame-ex").excoloSlider({
            ...
            pagerNav:  {/literal}{if $config.pager}false{else}true{/if}{literal},
