...
<nav id="topnavbar" class="navigation navbar yamm navbar-inverse {if $smarty.const.MENU_LOCATION}navbar-{$smarty.const.MENU_LOCATION}{/if}" role="navigation">
    <div class="">
        ...
        <div class="collapse navbar-collapse" id="bs-navbar-collapse-1">
            <ul class="nav navbar-nav{if $smarty.const.MENU_ALIGN == 'right'} navbar-right pull-right{/if}">
                ...
                {bootstrap_navbar menu=$hierarchy length=1}
            </ul>
            <div class="navbar-right" style="margin-top: 9px;">
                {showmodule controller=search action=show source=top-search chrome=true}
            </div>
            <div class="pull-right" style="margin-top: 15px;margin-right: 12px;">
                <a class="navbar-link loginpopup" href="{link controller=login action=showlogin}" title="{'Log in to the Exponent CMS web site'|gettext}" style="text-decoration: none;">{if $user->isLoggedIn()}{'Log Out'|gettext}{else}{'Log In'|gettext}{/if}</a>
            </div>
        </div>
    </div>
</nav>
