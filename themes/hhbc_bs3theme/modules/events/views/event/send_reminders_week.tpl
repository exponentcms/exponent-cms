...
<div class="module events cal-default">
	...
	<dl class="viewweek">
		{foreach from=$days item=events key=ts}
			{if $counts[$ts] != 0}
				<dt>
					<strong>
						<a class="itemtitle{if !empty($item->color)} {$item->color}{/if}" href="{link controller=event action=showall view=showall_Day time=$ts src=$src}" title="{'Click to display in browser'|gettext}">{$ts|format_date:"%A, %b %e"}
                            {if $config.week_long}
                                {$numdays = $config.numdays}
                                {if !$numdays}
                                    {$numdays = 7}
                                {/if}
                                {$endts = $ts + (86400 * $numdays)}
                                {'thru'|gettext} {$endts|format_date:"%A, %b %e"}
                            {/if}
                        </a>
					</strong>
				</dt>
    ...
    {br}
    <h3 align="center">7 Day Forecast for HHBC</h3>
    {br}
    {wx}
</div>